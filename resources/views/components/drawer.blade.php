...
<div class="drawer z-10">
    ...
    <div class="drawer-side">
        ...
        <ul class="menu menu-lg bg-base-200 text-base-content min-h-full w-72 p-4 dark:bg-gray-800">
            ...
            @if (auth()->user())
                <li>
                    <h2 class="my-2 text-lg font-bold text-gray-800 dark:text-gray-400" onclick="showDetail(-1);">
                        {{ __('Yêu thích') }}</h2>
                </li>
                <li>
                    <h2 class="my-2 text-lg font-bold text-gray-800 dark:text-gray-400" onclick="showDetail(-2);">
                        {{ __('Tính chi tiêu') }}</h2>
                </li>
            @endif
            <li>
                    ...
                    @if (isset($listItems))
                        @foreach ($listItems as $item)
                            <li class="flex flex-row items-center gap-2">
                                <x-actions.sidebar-link :href="Route::has($item['route'] ?? '') ? route($item['route']) : '#'" :active="false"
                                ...
                                <a class="{{ array_key_exists('id', $item) ? '' : 'hidden' }}" href="#"
                                    onclick="openEditTourModal({{ array_key_exists('id', $item) ? $item['id'] : null }})"
                                    class="mb-2">
